...
# $featureCounts  -p --extraAttributes gene_name -a $gtf -o $output $bam_new

samtools view ${outdir}/results.hisat_align.bam.featureCounts.bam.assigned.bam | awk '{print $1"\t"$NF}' | sort | uniq > ${outdir}/genes.to.reads.txt

use R-3.5
export R_LIBS_USER=/psych/genetics_data/ssimmons/R/x86_64-pc-linux-gnu-library/3.5

Rscript /ahg/regevdata2/projects/sc_compare2/seanken/Pipelines/SCUMI/PBMC/Count.results.R $outdir 
