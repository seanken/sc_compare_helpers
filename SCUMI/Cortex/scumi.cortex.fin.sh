...
#$ -cwd
#$ -e ErrFile/error.out2.err
#$ -o ErrFile/out.out2.log
#$ -q broad
#$ -P regevlab
#$ -l h_vmem=90g
#$ -l h_rt=140:00:00
...
#samples=$1
samples=cort_files/Cortex.samples.txt
SEEDFILE=$samples
...
index_dir=/ahg/regevdata/projects/sc_compare/ref/cellranger/refdata-cellranger-mm10-2.1.0
...
echo Tag bam!
scumi tag_bam $bam --gtf $gtf --featureCounts $feature_count_dir"/featureCounts" --annotate_intron --annotate_multi_mapping

# echo Clean yet again!
# rm $bam

echo Count!
scumi count_umi --bam $bam".featureCounts.bam" --molecular_info_h5 $molecular_info_h5 --cell_barcode_count $cell_barcode_count --gtf $gtf --expect_cell $numCells --force_cell $possible_barcodes 
