...
fastq1_newer=$outdir/results.1.fastq
fastq2_newer=$outdir/results.2.fastq
...
#seqtk sample -s 100 $fastq1 $ratio   > $fastq1_new
#seqtk sample -s 100 $fastq2 $ratio   > $fastq2_new

zcat $fastq1_list > $fastq1_newer
zcat $fastq2_list > $fastq2_newer
...
ref=/ahg/regevdata2/projects/sc_compare2/seanken/ref/mm10/RSEM/mouse_ref
rsem-calculate-expression --paired-end $fastq1_newer $fastq2_newer  --time --seed 555  --single-cell-prior $ref $outdir/rsem
